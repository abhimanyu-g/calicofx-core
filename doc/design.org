...
#+begin_src plantuml :file images/calicofx-plugin-base-class-dig.png :exports results
  @startuml
  struct portDesc{
  	string label
  	uint8_t index
  ...
  class PluginBase{
  	...
  	+virtual int pluginUpdateParam(uint8_t idx, int val) = 0
  }
  @enduml
#+end_src
*** LV2-manager
...
#+begin_src plantuml :file images/calicofx-lv2-manager-class-dig.png :exports results
  @startuml
  class LV2Manager: public PluginBase{
  	...
  	+int pluginUpdateParam(uint8_t idx, int val) @override
  }
  @enduml
#+end_src
